...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseSqlServer("Data Source=LAPTOP-7HORMRAF\\SQLEXPRESS01;Initial Catalog=CuaHangBanHang;Integrated Security=True;Trust Server Certificate=True");
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.HasKey(e => e.IdHoadon).HasName("PK__HOADON__018FF0B50E48D587");

            entity.HasOne(d => d.IdKhachhangNavigation).WithMany(p => p.Hoadons).HasConstraintName("FK__HOADON__ID_KHACH__5441852A");

            entity.HasOne(d => d.IdKhuyenmaiNavigation).WithMany(p => p.Hoadons).HasConstraintName("FK__HOADON__ID_KHUYE__5535A963");

            entity.HasOne(d => d.IdNhanvienNavigation).WithMany(p => p.Hoadons).HasConstraintName("FK__HOADON__ID_NHANV__5629CD9C");
        });

        modelBuilder.Entity<Hoadonct>(entity =>
        {
            entity.HasKey(e => e.IdHoadonct).HasName("PK__HOADONCT__CB5C3B20EC890F86");

            entity.HasOne(d => d.IdHoadonNavigation).WithMany(p => p.Hoadoncts).HasConstraintName("FK__HOADONCT__ID_HOA__59FA5E80");

            entity.HasOne(d => d.IdSanphamctNavigation).WithMany(p => p.Hoadoncts).HasConstraintName("FK__HOADONCT__ID_SAN__59063A47");
        });
